...
import { Prisma, type SiteInstall, type SiteInstallSession, type Tenant } from "@prisma/client";
...
function getTenantAllowedOrigins(tenant: Pick<Tenant, "allowedOrigins" | "domains">) {
  if (tenant.allowedOrigins.length > 0) {
    return tenant.allowedOrigins.map((origin) => normalizeInstallOrigin(origin).origin);
  }
...
export async function ensureDefaultScriptInstall(input: {
  tenantId: string;
  ...
  const existing = await prisma.siteInstall.findUnique({
    where: {
  ...
  return prisma.siteInstall.create({
    data: {
      tenantId: input.tenantId,
      platform: "script",
      label: `${domain} embed`,
...
export async function provisionSiteInstall(input: {
  tenantId: string;
  origin: string;
  ...
  if (existing) {
    ...
    return prisma.siteInstall.update({
...
export async function resolveBootstrapInstall(input: {
  ...
  if (input.installKey) {
    const siteInstall = await prisma.siteInstall.findUnique({
  ...
  const install =
    (await prisma.siteInstall.findUnique({
      where: {
        tenantId_origin_platform: {
          tenantId: tenant.id,
          origin,
...
export async function issueVisitorSession(input: {
  install: SiteInstall;
  tenantId: string;
  origin: string;
  visitorToken?: string;
  ...
  const existingTokenHash = input.visitorToken ? hashToken(input.visitorToken) : null;
  ...
  if (
    ...
    existingSession = await prisma.siteInstallSession.findFirst({
      where: {
  ...
  if (existingSession) {
    await prisma.siteInstallSession.update({
    ...
    await prisma.siteInstallSession.create({
      data: {
        tenantId: input.tenantId,
        siteInstallId: input.install.id,
        sessionId,
...
export async function authenticateVisitorRequest(request: Request) {
  ...
  await Promise.all([
    prisma.siteInstallSession.update({
      where: { id: session.id },
      data: {
        lastSeenAt: new Date(),
...
export async function createWordPressInstallLinkCode(input: {
  tenantId: string;
  origin: string;
  returnUrl: string;
  ...
  await prisma.siteInstallLinkCode.create({
    data: {
      tenantId: input.tenantId,
...
export async function exchangeWordPressInstallLinkCode(input: {
  ...
  const linkCode = await prisma.siteInstallLinkCode.findUnique({
    where: { codeHash },
  ...
  await Promise.all([
    ...
    prisma.siteInstallLinkCode.update({
...
export async function authenticateManagedInstall(input: {
  ...
  const install = await prisma.siteInstall.findUnique({
    where: { publicInstallKey: input.installKey },
  });
