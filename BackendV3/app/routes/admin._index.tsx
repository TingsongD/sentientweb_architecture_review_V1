...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [
    ...
    Promise.all([
      ...
      db.lead.count({ where: { tenantId: tenant.id, bookingEligible: true } }),
      db.demoBooking.count({
        where: { tenantId: tenant.id, status: "booked" },
      }),
  ...
  return {
    ...
    summary: {
      ...
      acceptanceRate:
        reviewedCount > 0
          ? Math.round((acceptedCount / reviewedCount) * 100)
          : null,
      reviewedCount,
    },
    knowledgeSources,
    ...
    <div className="stack">
      <section className="hero-card stack">
        ...
        <p className="muted" style={{ maxWidth: 720 }}>
          This dashboard tracks the live Phase 1 loop: knowledge retrieval,
          qualification, bookings, and webhook delivery for the current tenant.
        </p>
        <div className="form-actions">
          <Link className="button" to="/admin/settings">
            Configure tenant
          </Link>
          <Link className="button-secondary" to="/admin/activity">
      ...
      <section className="grid">
        ...
        <div className="metric">
          ...
          <div className="muted">{data.summary.reviewedCount} reviewed</div>
        </div>
      </section>
      ...
      <section className="grid two">
        <article className="panel stack">
          ...
          >
            ...
            <Link to="/admin/settings" className="button-secondary">
              Manage
            </Link>
          </div>
          {data.knowledgeSources.length === 0 ? (
            <p className="muted">
              No crawls yet. Start by adding the docs or marketing site URL in
              settings.
            </p>
          ) : (
