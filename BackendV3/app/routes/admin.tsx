...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const primaryInstall = await prisma.siteInstall.findFirst({
    where: {
      tenantId: tenant.id,
      status: "active",
    },
    ...
    <main className="page stack">
      <header className="site-nav">
        <div className="stack" style={{ gap: 4 }}>
          ...
          <div className="muted">
            Install key: <span className="mono">{data.tenant.primaryInstallKey ?? "pending"}</span>
          </div>
        </div>
        <nav>
          ...
          <NavLink to="/admin/activity" className={({ isActive }) => (isActive ? "active" : undefined)}>
            Activity
          </NavLink>
          <NavLink to="/admin/settings" className={({ isActive }) => (isActive ? "active" : undefined)}>
            Settings
          </NavLink>
          <NavLink to="/admin/installs" className={({ isActive }) => (isActive ? "active" : undefined)}>
            Installs
          </NavLink>
        </nav>
      </header>
      ...
      <section className="panel stack">
        <div className="muted">Signed in as {data.admin.fullName ?? data.admin.email}</div>
